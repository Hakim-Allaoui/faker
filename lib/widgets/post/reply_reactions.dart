...
class ReplyReactions extends StatefulWidget {
  ...
  const ReplyReactions({Key key, this.reply, this.notify, this.onChanged}) : super(key: key);
...
class _ReplyReactionsState extends State<ReplyReactions> {
  ...
  void initState() {
    ...
    for (int i = 0; i < 7; i++) {
      if (!widget.reply.replyReactions.reactions.contains(reactionsList[i]))
  ...
  Widget build(BuildContext context) {
    return InkWell(
                                  ...
                                  (item) => InkWell(
                                    ...
                                    onTap: () {
                                      if (!reactState[
                                          ...
                                          widget.reply.replyReactions.reactions
                                                  .length >
                                              0) {
                                        if ((widget.reply.replyReactions
                                                    .reactions.length <
                                                3 ||
                                            reactState[
                                                reactionsList.indexOf(item)])) {
                                          dialogSetState(() {
                                            ...
                                            if (reactState[
                                                reactionsList.indexOf(item)])
                                              widget.reply.replyReactions
                                                  .reactionsCount++;
                                            else if (widget.reply.replyReactions
                                                    .reactionsCount >
                                                0)
                                              widget.reply.replyReactions
                                                  .reactionsCount--;
                          ...
                          child: Center(
                            child: TextField(
                              ...
                              decoration: InputDecoration(
                                ...
                                hintText: widget
                                    .reply.replyReactions.reactionsCount
                                    .toString(),
      ...
      child: widget.reply.replyReactions.reactionsCount == 0
          ? SizedBox()
          : Container(
              child: Row(
                mainAxisSize: MainAxisSize.min,
                  ...
                  Padding(
                    ...
                    child: Text(
                      NumberFormat.compactCurrency(decimalDigits: 0, symbol: '')
                          .format(widget.reply.replyReactions.reactionsCount)
                          .toString(),
                      style: MyTextStyles.fbText.apply(color: Palette.greyDark),
                    ),
                  ),
                  widget.reply.replyReactions.reactions.isEmpty
                      ? SizedBox()
                      : Container(
                          child: Stack(
                            alignment: Alignment.centerLeft,
                            children: widget
                                .reply.replyReactions.reactions.reversed
                                .map((item) {
                              return Container(
                                margin: EdgeInsets.only(
                                    left: widget.reply.replyReactions.reactions
                                            .indexOf(item)
                                            .toDouble() *
                                        14.0),
                                decoration: BoxDecoration(
  ...
  void _updateReactions() {
    setState(() {
      widget.reply.replyReactions.reactions.clear();
      for (int i = 0; i < reactionsList.length; i++) {
        if (reactState[i])
          widget.reply.replyReactions.reactions.add(reactionsList[i]);
      }
    });
    print('==============> ${widget.reply.replyReactions.reactions}');
  }
